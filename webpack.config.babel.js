  ...
  return {
    entry: {
      app: [
        ...
        isWechat && `es6-promise/dist/es6-promise.auto${isDev ? '.min' : ''}.js`,
        './src/app.js'
      ].filter(Boolean)
    },
    output: {
    ...
    module: {
      rules: [
        ...
        },
        {
          ...
          include: /src/,
          exclude: /node_modules/,
          use: [relativeFileLoader(), 'babel-loader', shouldLint && 'eslint-loader'].filter(Boolean)
        },
        {
          test: /\.(less|wxss|acss)$/,
          include: /src/,
          use: [
            relativeFileLoader(isWechat ? 'wxss' : 'acss'),
            {
              loader: 'less-loader',
              options: {
                paths: [resolve('src', 'styles'), srcDir]
              }
            }
          ]
        },
